...
class QuoteManager(models.Manager):
    def validator(self, POST):
        ...
        if len(POST['content']) < 11:
            errors['content'] = 'Quote should be more than 10 characters.'
        return errors

class Quote(models.Model):
    author = models.CharField(max_length=255)
    content = models.CharField(max_length=245)
    poster = models.ForeignKey(User, related_name = 'contributer', on_delete = models.CASCADE)
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
